...
class LanguageLevelWidget extends ConsumerStatefulWidget {
  const LanguageLevelWidget({super.key});

  @override
  ConsumerState<LanguageLevelWidget> createState() =>
      _LanguageLevelWidgetState();
}

class _LanguageLevelWidgetState extends ConsumerState<LanguageLevelWidget> {
  Timer? _tickTimer;
  ...
  Widget build(BuildContext context) {
    final profileAsync = ref.watch(userLanguageProfileProvider);
    ...
    return profileAsync.when(
      ...
      data: (profile) {
        ...
        // Calculate Time Remaining for next paw
        String? timeDisplay;
        if (pawState?.nextRegenTime != null) {
          final remaining = pawState!.nextRegenTime!.difference(DateTime.now());
          if (remaining.isNegative) {
            timeDisplay = "00:00";
          } else {
            timeDisplay = _formatTime(remaining);
          }
        }
              ...
              if (pawState != null)
                Row(
                  children: [
                    ...
                    if (timeDisplay != null &&
                        pawState.count < pawState.maxCount) ...[
